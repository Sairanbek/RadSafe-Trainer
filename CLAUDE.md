...
`correct_letter` клиенту сразу.

У вопросов внешних банков есть готовый `explanation` (и `source`) — клиенты
показывают его вместо запроса к Gemini. Отдаётся он только там, где правильный
ответ уже раскрыт: в `QuestionOut` при `mode == "learning"` и в `AnswerOut`
после ответа. **Не добавляйте `explanation` в выдачу вопроса других режимов** —
это покажет ответ до выбора варианта. Длина тренировки зависит от размера раздела
